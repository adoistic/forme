...
test(
  "big issue: 20 articles via NewArticleModal + 120 classifieds + every ad position",
  async () => {
    ...
    await window
      ...
      .fill("Saptahik Weekly — Big Issue");
    ...
    const mdFiles = (await fs.readdir(ARTICLES_MD_DIR))
    ...
    for (const file of mdFiles) {
      ...
      await window
        .getByTestId("new-article-content-type")
        .selectOption(article.contentType);
    ...
    await window
      .getByTestId("import-csv-input")
      .setInputFiles(CLASSIFIEDS_CSV);
