...
const BODY_BOT_INCH = 10.9; // trim - marginBottom
const FOLIO_TOP_INCH = 11.3; // folio sits ~here
const LEADING_PT = 14;
...
async function analyze(pngPath: string, page: number): Promise<PageStats> {
  ...
  if (avgFill < 0.7 && type === "body") flags.push("under-filled");
  // Cols uneven (>20% spread)
  if (Math.max(...colFill) - Math.min(...colFill) > 0.2) flags.push("uneven");
...
async function main() {
  ...
  console.log(
    `  overflow into footer: ${issues.overflow.length} → pages [${issues.overflow.map((p) => p.page).join(",")}]`
  );
  console.log(
    `  under-filled body (<70%): ${issues.underFilled.length} → pages [${issues.underFilled.map((p) => p.page).join(",")}]`
  );
  console.log(
    `  uneven cols (>20% spread): ${issues.uneven.length} → pages [${issues.uneven.map((p) => p.page).join(",")}]`
  );
  console.log(
    `  internal mid-col gap (>5 lines empty): ${issues.internalGap.length} → pages [${issues.internalGap.map((p) => p.page).join(",")}]`
  );

  const avgFillBody =
    bodyPages.reduce((a, p) => a + (p.colFill[0]! + p.colFill[1]! + p.colFill[2]!) / 3, 0) /
    bodyPages.length;
